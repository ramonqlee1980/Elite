...
-(NSString*)mobisageId
{
    ...
    return (mobisageId&&mobisageId.length>0)?mobisageId:kDefaultMobisageId;
}

-(NSString*)youmiAppId
{
    static NSString* youmiAppId = kDefaultYoumiAppId;
...
{
    static NSString* youmiSecret = kDefaultYouSecret;
...
-(BOOL)keyEnabled:(NSString*)key
{
    BOOL keyEnabled = NO;
    for (NSInteger i = 0; i< [self.adsArray count]; ++i) {
        NSDictionary* dict = [self.adsArray objectAtIndex:i];
        NSString* owner = [dict objectForKey:kOwerTag];
        if ([key caseInsensitiveCompare:owner]==NSOrderedSame) {
            keyEnabled = [kEnabledString isEqualToString:[dict objectForKey:kEnabledString]];
            break;
        }
    }
    return keyEnabled;
...
-(BOOL)IAPEnabled
{
//    return [self keyEnabled:kIAPKey];
    return YES;
}
-(BOOL)recommendViewInSettingEnabled
{
    return  [self keyEnabled:kRecommendViewKey];
}

-(NSInteger)RecommendAdPopTimes
{
#define kDefaultRecommendPopTimes 3//configured on server
    for (NSInteger i = 0; i< [self.adsArray count]; ++i) {
        NSDictionary* dict = [self.adsArray objectAtIndex:i];
        NSString* owner = [dict objectForKey:kOwerTag];
        if ([kRecommendViewPopTimesKey caseInsensitiveCompare:owner]==NSOrderedSame) {
            NSString* count = (NSString*)[dict objectForKey:kCountKey];
            return [count integerValue];
        }
    }
    return kDefaultRecommendPopTimes;
}
@end
